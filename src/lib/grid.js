...
import { TweenMax, Expo, Elastic, Back, Power1, Power3 } from "gsap";
...
export function grid() {
  ...
  class GridItem {
    ...
    toggleTexts(delay, withAnimation, show = true) {
      TweenMax.to(
        ...
        {
          ease: show ? Expo.easeOut : Power3.easeIn,
  ...
  class Content {
    constructor(el) {
      ...
      this.DOM.content = this.DOM.el.querySelector(".content__item-content");
        ...
        () => {
          this.DOM.titleLetters.forEach((letter, pos) => {
            TweenMax.to(letter, !withAnimation ? 0 : show ? 0.6 : 0.3, {
              ease: show ? Back.easeOut : Power3.easeIn,
              delay: !withAnimation
                ? 0
                : show
                ? pos * 0.05
          ...
          this.DOM.content.style.opacity = show ? 1 : 0;
        },
  ...
  class Grid {
    ...
    openItem(item) {
      ...
      item.hideTexts();
      // Set the itemÂ´s z-index to a high value so it overlaps any other grid item.
      ...
      // Calculate the viewport diagonal. We will need to take this in consideration when scaling up the itemÂ´s bg element.
      const d = Math.hypot(winsize.width, winsize.height);
      // Scale up the itemÂ´s bg element.
      ...
      // Calculate the itemÂ´s image and contentÂ´s image sizes and positions.
      // We need this so we can scale up and translate the itemÂ´s image to the same size and position of the contentÂ´s image.
      const imgDim = this.getSizePosition(item.DOM.imgWrap);
      const contentImgDim = this.getSizePosition(contentEl.DOM.img, false);
      // Show the back control and scroll indicator and all the itemÂ´s content elements (1 second delay).
      this.showContentElems(contentEl, 1);
      // Animate the itemÂ´s image.
      TweenMax.to(item.DOM.tilt.img, 1.2, {
        ...
        rotation: 0,
        onComplete: () => {
          // Hide the itemÂ´s image and show the contentÂ´s image. Should both be overlapping.
          item.DOM.tilt.img.style.opacity = 0;
          contentEl.DOM.img.style.visibility = "visible";
          // Set the main content wrapper to absolute so itÂ´s position at the top.
    ...
    closeItem(withAnimation = true) {
      ...
      // Hide the back control and scroll indicator and all the itemÂ´s content elements.
      this.hideContentElems(contentEl, 0, withAnimation);
      // Set the gridÂ´s image back to visible and hide the contentÂ´s one.
      item.DOM.tilt.img.style.opacity = 1;
      contentEl.DOM.img.style.visibility = "hidden";
      // Animate the gridÂ´s image back to the grid position.
    ...
    toggleContentElems(contentEl, delay, withAnimation, show = true) {
      ...
      const scrollIndicator = this.DOM.scrollIndicator;
      setTimeout(function() {
        scrollIndicator.classList.toggle("animated");
